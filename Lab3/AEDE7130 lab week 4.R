...
pacman::p_load(car, dplyr, fixest, stargazer, sensemakr,
...
uganda_sub <- ugandadata %>% data.frame %>%
  mutate(logfarmrev=log(sfarmrev)) %>%
  dplyr::select(logfarmrev, avghrsperwker, farmsize, 
                hhsize, extvisits, agehead, hhavgeduc, 
                fertilizer, seedtechs, malehead, distance, drought, urban,
                year, hhid, comm)

ols1 <- feols(logfarmrev ~ fertilizer, data=uganda_sub, vcov="HC1")
ols2 <- feols(logfarmrev ~ fertilizer | hhid, data=uganda_sub, vcov="HC1")
ols3 <- feols(farmsize ~ fertilizer + hhsize+malehead+seedtechs | hhid, data=uganda_sub, vcov="HC1")

# extract p-value
p1   <- ols1$coeftable["fertilizer","Pr(>|t|)"]   
...
sensitivity_test<-sensemakr(model = ols5,
                            treatment = "educ",
...
ovb_minimal_reporting(sensitivity_test,  format = "latex")
